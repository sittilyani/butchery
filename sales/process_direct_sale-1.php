...
try {
    ...
    foreach ($items_list as $item) {
        ...
        if ($product_result->num_rows > 0) {
            ...
            // If product not found, try to get basic info without stock balance
        ...
        // Insert into stocks table for sales transaction
        ...
        // Also update stock_movements table if it exists
        ...
        $stock_movements_stmt->bind_param("issiiis", $product_id, $item['brandname'], $productname, $latest_stockBalance, $item['quantity'], $new_stockBalance, $transBy);
    ...
    if (SAVE_PDF_IF_POSSIBLE && (strtolower($payment_status) === 'paid' || strtolower($payment_status) === 'credit')) {
        ...
        foreach ($items_list as $item) {
            $brandname = substr($item['brandname'], 0, 20); // Limit product name length
            $quantity = str_pad($item['quantity'], 3, ' ', STR_PAD_LEFT);
            $price = str_pad(number_format($item['price'], 2), 8, ' ', STR_PAD_LEFT);
            $discountAmt = str_pad(number_format($item['discount'], 2), 6, ' ', STR_PAD_LEFT);
            $total = str_pad(number_format($item['grand_total'], 2), 10, ' ', STR_PAD_LEFT);

            $itemsHtml .= "<div style='font-family: monospace; font-size: 9px; line-height: 1.1;'>";
        ...
        // Convert logo to base64 to ensure it displays
        ...
        <body>
            <div class='header'>
                " . ($logo_data ? "<div class='logo'><img src='$logo_base64' alt='Logo'></div>" : "") . "
            ...
            <div class='footer'>
                <div>Thank you for your consultation!</div>
        ...
        try {
            ...
            $dompdf->setPaper([0, 0, 234, 1000], 'portrait'); // Width: ~72mm in points
            $dompdf->render();
