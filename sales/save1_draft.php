...
try {
    ...
    foreach ($data['items'] as $item) {
        ...
        if ($stockBalance <= 0 || $stockBalance < $quantity) {
            throw new Exception("No enough stocks to sale for $product_name");
        }
    }
    ...
    $draft_stmt = $conn->prepare("
        INSERT INTO sales_drafts (
            receipt_id, product_name, quantity, price, total_amount,
    ...
    foreach ($data['items'] as $item) {
        $required_item_fields = ['product_name', 'quantity', 'price', 'total_amount', 'tax_amount', 'grand_total'];
        ...
        $stock_stmt->bind_param(
            "isdis",
            $category_id, $product_name, $quantity, $newStockBalance, $transBy, $reorder_level
        );
        $stock_stmt->execute();
    }
