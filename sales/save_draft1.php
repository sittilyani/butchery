...
try {
    ...
    foreach ($data['items'] as $item) {
        ...
        if ($stockBalance <= 0 || $stockBalance < $quantity) {
            throw new Exception("No enough stocks to sale for $productname");
        }
    }
    ...
    $stock_stmt = $conn->prepare("
        INSERT INTO stocks (
            category_id, productname, quantityOut, stockBalance, transBy, reorderLevel
        ) VALUES (?, ?, ?, ?, ?, ?)
    ");
    ...
    foreach ($data['items'] as $item) {
        ...
        $product_query = $conn->prepare("SELECT id, reorder_level FROM products WHERE productname = ?");
