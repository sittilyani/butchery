...
$sql = "
    SELECT s1.id, s1.brandname, COALESCE(p.price, 0.00) AS price, s1.stockBalance, s1.expiryDate, s1.status
    FROM stocks s1
    INNER JOIN (
...
if (!$result) {
    die("Error fetching stocks: " . $conn->error);
}
            ...
            <table id="stocks-table">
                ...
                <tbody>
                    <?php foreach ($stocks as $index => $stock): ?>
                        <?php
                        // Validate expiryDate on the server-side
                        ...
                        <tr class="product-row <?php echo $index >= 10 ? 'hidden' : ''; ?> <?php echo !$isValidExpiry ? 'invalid' : ''; ?>"
                            ...
                            data-stock-balance="<?php echo htmlspecialchars($stock['stockBalance']); ?>"
                            data-expiry-date="<?php echo htmlspecialchars($stock['expiryDate'] ?: ''); ?>">
                            ...
                            <td data-label="Stock Balance"><?php echo htmlspecialchars($stock['stockBalance']); ?></td>
    ...
    $(document).ready(function() {
            ...
            if (draftId && parseInt(draftId) > 0) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        showError('Error removing item: ' + xhr.responseText);
                    }
                });
            }
            updateOrderTable();
            ...
            orderItems.forEach((item, index) => {
                ...
                html += `
                    <tr>
                        ...
                        <td><input type="number" class="form-control discount-input" data-draft-id="${item.draft_id || item.product_id}" value="${parseFloat(item.discount).toFixed(2)}" step="0.01" min="0"></td>
        ...
        function processPayment(paymentType, customerName = '', customerPhone = '') {
            const data = {
                ...
                total_discount: $('#total-discount').text(),
        ...
        $('#print-receipt').click(function() {
            ...
            orderItems.forEach((item, i) => {
                itemsHtml += `
                    <tr>
                        ...
                        <td style="padding:2px;border:1px solid #000;">${parseFloat(item.grand_total).toFixed(2)}</td>
                    </tr>`;
            });
            ...
            printWindow.document.write(`
                ...
                <div class="totals">
                    ...
                    <p><span style="font-weight: bold; font-size: 16px; color: red;">Joima Pharmaceuticals</span></p>
