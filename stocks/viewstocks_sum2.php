...
$(document).ready(function() {
    ...
    $('#product-search').on('input', function() {
        ...
        typingTimer = setTimeout(function() {
            ...
            $.ajax({
                url: 'viewstocks_sum.php', // The same script
                method: 'GET',
                data: { search: search },
                success: function(response) {
                    $('#stocks-table').html(response);
                    $spinner.hide();
                    $icon.show();
                },
                error: function(xhr, status, error) {
                    $('#stocks-table').html('<tr><td colspan="6" class="text-center text-danger">Error fetching data. Please try again.</td></tr>');
